...
API_SEMAPHORE  = threading.Semaphore(25)  # 글로벌 NAMC API 동시 한도 (NAMC 가이드 50, 실측 40은 503 발생 → 25로 보수적 운영)
...
**핵심 질문**: [이미지 2]가 위 AI 프롬프트의 **핵심 시그니처 요소**를 가지고 있나요?

판별 원칙:
1. 프롬프트에는 보통 여러 카테고리의 시그니처가 있음 — **소품, 포즈, 장면, 구도, 조명, 색감, 카메라 스타일** 등.
   이 중 **서로 다른 카테고리 2개 이상**이 [이미지 2]에 명확히 보이면 → YES
2. **단 하나의 시그니처(특히 UI 오버레이/그래픽 요소/특정 텍스트)만 빠졌다고 NO 하지 마세요.**
   예: 프롬프트가 "팬싸인회 포즈 + 검은 테이블 + DSLR UI 오버레이"를 명시했고,
   [이미지 2]에 팬싸인회 포즈 + 검은 테이블은 있지만 UI 오버레이만 없으면 → 여전히 YES (2개 카테고리 매치)
3. **유지 요소(헤어, 의상, 얼굴, 배경)**의 차이는 무시 — 어차피 유저별로 다름
4. 의심스러우면 NO. 하지만 "헤어 다르니까 NO" 또는 "UI 오버레이 없으니까 NO" 같은 단일 요소 판단은 하지 마세요

**자주 오판하는 케이스 (관대 모드에서도 NO)**:
- 어느 카테고리의 시그니처도 매치되지 않음
- 단순한 셀카/AI풍이라는 표면적 공통점만
- 핵심 시그니처와 무관한 이미지

**자주 오판하는 케이스 (관대 모드는 YES로 봐야 함)**:
- 2개 이상 카테고리 시그니처 매치되는데 UI/오버레이/그래픽 1개만 없는 경우 → YES
- 포즈·구도·소품·장면이 매치되는 실제 사진 (AI 생성물 아닌 진짜) → YES

확신 안 서면 NO. 카테고리 다양성에 집중. 반드시 YES 또는 NO 한 단어만 답하세요."""
    ...
    try:
        ...
        for uname, p in profile_map.items():
            ...
            for lp in sorted_posts[:2]:                          # 최신 2개 게시글 (핀 포함) — 게시 다음날 검수 워크플로우 기준
                ...
                # 캐러셀(Sidecar)이면 images[] 모두, 아니면 displayUrl 1장
